...
### Fixed
- **Performance**: Skip question-level navigation recomputation in stepped/classic modes. `computeQuestionState()` and `getVisibleQuestions()` now only run when `displayMode` is `"conversational"`, eliminating redundant iteration on every keystroke.
