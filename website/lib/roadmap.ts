...
export const roadmap: RoadmapGroup[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { title: 'Analytics event system (onEvent callback)', status: 'shipped' },
      { title: 'Readonly field state with conditional expressions', status: 'shipped' },
      { title: 'beforeSubmit hook for response interception', status: 'shipped' },
      { title: '25 condition operators (dateAfter, arrayContains, matchesRegex, etc.)', status: 'shipped' },
      { title: 'Expression functions (UPPER, LOWER, TODAY, DATEDIFF, IF, etc.)', status: 'shipped' },
      { title: 'Conditional validation with applyIf and severity levels', status: 'shipped' },
      { title: '6 new validators (integer, positiveNumber, alphanumeric, compareToField, etc.)', status: 'shipped' },
      { title: 'Draft auto-save, versioning, and migration support', status: 'shipped' },
      { title: 'Schema utilities (diff, merge, clone, migrate)', status: 'shipped' },
      { title: 'Form data export (JSON, CSV, flat)', status: 'shipped' },
      { title: '6 new React hooks (useFormDirty, useFormSubmit, useFieldVisibility, etc.)', status: 'shipped' },
      { title: '/testing and /validators subpath exports', status: 'shipped' },
      { title: 'SECURITY.md and FUNDING.yml', status: 'shipped' },
