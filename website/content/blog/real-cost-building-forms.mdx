...
- 19 built-in validators (required, min, max, email, phone, URL, date, pattern, file size, file type, integer, positiveNumber, alphanumeric, noSpecialChars, minItems, maxItems, compareToField, length constraints)
...
All eight systems — validation, conditional logic, multi-step, theming, adapters, prefill, draft persistence, and 42 field types — ship with that import. The schema defines your form. The engine handles everything else.
