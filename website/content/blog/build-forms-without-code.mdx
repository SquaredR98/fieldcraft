...
> **TL;DR** — FormBuilder is a visual drag-and-drop interface that outputs valid FieldCraft JSON schemas. Non-developers can create production-ready forms without writing code. Part of FieldCraft Pro.
...
The left side of the screen is a palette of all 44 field types. Short text, email, phone, dropdown, date picker, file upload, rating, signature pad, Likert scale, NPS. You drag a field type from the palette onto the canvas and it appears as a block you can reorder, duplicate, or delete.
...
Every one of the 44 field types has a dedicated configuration UI. This isn't a generic "enter JSON" fallback. The panel knows what each field type supports and shows exactly those options.
...
> **Note:** FormBuilder is part of FieldCraft Pro, which is not yet published. The code examples below show the planned API.
...
```tsx
// Pro API preview — @squaredr/fieldcraft-pro is not yet published
import { FormBuilder } from '@squaredr/fieldcraft-pro/form-builder'
...
## Part of FieldCraft Pro (Coming Soon)

FormBuilder will be part of the FieldCraft Pro toolkit alongside SchemaEditor and ResponseViewer. Pro is currently in development — star the [GitHub repo](https://github.com/SquaredR98/fieldcraft) to get notified when it ships.
