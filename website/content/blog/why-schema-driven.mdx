...
| **React Renderer** | `@squaredr/fieldcraft-react` | Renders 41 field types, manages focus and keyboard nav, displays errors and progress |
...
There's a `createHttpSchemaAdapter` that lets you load schemas from any REST endpoint:
...
The engine supports 41 field types out of the box. For the remaining edge cases, you can register custom field types through the field registry and they'll work with all the same validation, conditional logic, and persistence features.
...
There are also 16 free production-ready templates if you want to see what a real schema looks like:
